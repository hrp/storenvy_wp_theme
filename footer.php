			
		
			<div id='container-footer'>
				<ul class="footer-links cart-links">
					<li>
						<a href="http://www.storenvy.com/how-it-works">How It Works</a>
					</li>
					<li>
						<a href="http://www.storenvy.com/trending">Popular Items</a>
					</li>	
					<li>
						<a href="http://www.storenvy.com/shop/?order=newest">Newest Items</a>
					</li>	
				</ul>
				<ul class="footer-links store-links">
					<li>
						<a href="http://www.storenvy.com/open-a-store">Open A Store</a>
					</li>
					<li>
						<a href="http://www.storenvy.com/search/stores">Best Selling Stores</a>
					</li>	
					<li>
						<a href="http://developers.storenvy.com/">Developers API</a>
					</li>	
				</ul>
				<ul class="footer-links envy-links">
					<li>
						<a href="http://www.storenvy.com/about/team">Team</a>
						<span class='separator'> · </span>
						<a href="http://www.storenvy.com/jobs">Jobs</a>
					</li>
					<li>
						<a href="http://blog.storenvy.com/">Blog</a>
					</li>	
					<li>
						<a href="http://www.storenvy.com/popup">SF Popup Shop</a>
					</li>	
				</ul>

			</div>


		</div> <?php // end #container ?>
		<footer id="footer" class='footer' role="contentinfo">

			<div id="inner-footer" class="wrap clearfix">
				<div class='left'>
					<img src="<?php bloginfo('template_url')?>/library/images/storenvy-logo-footer.png">
					<p class="source-org copyright">&copy; <?php echo date('Y'); ?> Storenvy Inc.</p>
				</div>
				<div class='right'>
					<ul class='footer-links'>
						<li>
							<a href="http://storenvy.s3.amazonaws.com/Storenvy%20Press%20Kit.zip">Press Kit</a>
						</li>
						<li class='separator'> · </li>
						<li>
							<a href="http://support.storenvy.com/customer/portal/articles/93985-terms-of-use">Terms of Service</a>
						</li>
						<li class='separator'> · </li>
						<li>
							<a href="http://support.storenvy.com/customer/portal/articles/93982-privacy-policy">Privacy Policy</a>
						</li>
						<li class='separator'> · </li>
						<li>
							<a href="http://support.storenvy.com/customer/portal/articles/485749">Creative Rights Policy</a>
						</li>
						<li class='separator'> · </li>
						<li>
							<a href="http://support.storenvy.com/customer/portal/emails/new">Contact Us</a>
						</li>
						<li class='separator'> · </li>
						<li>
							<a href="http://support.storenvy.com/">Support & FAQ</a>
						</li>
					</ul>
				</div>

			</div> <?php // end #inner-footer ?>

		</footer> <?php // end footer ?>
		
		

		<?php // all js scripts are loaded in library/bones.php ?>
		<?php wp_footer(); ?>
		

	</body>

</html> <?php // end page. what a ride! ?>
